...
The [Payload Parser](../payload-parser/index) was created to handle raw payloads sent by devices in order to extract measured variables or execute simple operations. The [Analysis](../analysis/index) feature is more powerful — it includes access to [Devices](../devices/index) and external services, but there is a cost to run analyses. Payload Parser uses a simple JavaScript engine developed by TagoIO to be easy, lightweight, and free.
...
- Use the [Payload Parser](../payload-parser/index) for simple, free parsing needs and [Analysis](../analysis/index) when you need external integration, advanced JavaScript features, or persistence across executions.
