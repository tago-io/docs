...
export default function ChangelogIndex({items}: {items: Array<{content: any}>}) {
  const location = useLocation();
  const history = useHistory();
  const [activeFilter, setActiveFilter] = useState('all');
  const [isReady, setIsReady] = useState(false);

  const parsePost = (c: any) => {
  ...
  const allowedFilters: string[] = ['all','tagoio-admin','tagoio-api','tagocore', ...(hasTagoDeploy ? ['tagodeploy'] : [])];
  ...
  const getProductIcon = (product: string) => {
    ...
    return (
      <img
        ...
        width={18}
        height={18}
        style={{ borderRadius: 4, filter: isBW ? 'grayscale(100%)' : undefined }}
      />
    );
  };
        ...
        <div className="row">
          <div className="col col--10 col--offset-1 changelog-page">
            ...
            <div className="changelog-chips margin-bottom--lg" role="tablist" aria-label="Filter changelog by product">
              ...
              <button className={`changelog-chip ${activeFilter === 'tagoio-admin' ? 'changelog-chip--active' : ''}`} onClick={() => setActiveFilter('tagoio-admin')} role="tab" aria-selected={activeFilter === 'tagoio-admin'}>
                TagoIO Admin
              </button>
              <button className={`changelog-chip ${activeFilter === 'tagoio-api' ? 'changelog-chip--active' : ''}`} onClick={() => setActiveFilter('tagoio-api')} role="tab" aria-selected={activeFilter === 'tagoio-api'}>
                TagoIO API
              </button>
                          ...
                          <article className="timeline-card card changelog-card">
                            <div className="timeline-card__header">
                              ...
                              <div className="timeline-card__meta">
                                <small className="text--secondary">{changelog.displayDate || changelog.date}</small>
                              </div>
                            </div>
